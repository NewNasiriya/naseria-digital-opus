/**
 * Gallery panel embedded in the Achievement editor.
 *
 * Manages rows in the `achievement_media` junction table. Mirrors the
 * News gallery panel but adds Arabic + English alt text (accessibility
 * requirement for the public achievement pages) and preserves any
 * legacy `image_url` values on rows that pre-date the Media Library.
 */
...
export function AchievementGalleryPanel({
  achievementId,
  disabled,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data, error } = await (supabase as any)
        ...
        .select(
          `id, media_id, caption_ar, caption_en, alt_ar, alt_en, display_order, image_url,
           media:media!achievement_media_media_id_fkey(bucket,storage_path,file_name,mime_type)`,
        )
      ...
      const enriched = await Promise.all(
        (data ?? []).map(async (r: GalleryRow) => {
          if (r.media && (r.media.mime_type ?? "").startsWith("image/")) {
            ...
            r._url = mediaPublicUrl({
              bucket: "external",
              storage_path: r.image_url,
            });
          }
          return r;
        }),
  ...
  if (!achievementId) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-surface-muted p-4 text-xs text-muted-foreground">
        احفظ الإنجاز كمسودة أولًا حتى تتمكن من إضافة صور المعرض.
      </div>
    );
  }
  ...
  const handleAdd = async (item: MediaItem) => {
    ...
    try {
      const { error } = await (supabase as any).from("achievement_media").insert({
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">
          صور المعرض التي تظهر داخل صفحة الإنجاز. لا تُغيّر ترتيب الصور الأصلية إلا عند الحاجة.
        </p>
        <Button
          type="button"
          size="sm"
          ...
          {rows.map((row, idx) => (
            ...
            >
              <div className="overflow-hidden rounded-lg border border-border bg-surface-muted">
                <AspectRatio ratio={16 / 10}>
                  {row._url ? (
                    <img
                      src={row._url}
                      alt={row.alt_ar ?? row.caption_ar ?? row.media?.file_name ?? ""}
              ...
              <Input
                ...
                onChange={(e) => patchField(row.id, "caption_ar", e.target.value)}
              ...
              <Input
                value={row.alt_ar ?? ""}
                onChange={(e) => patchField(row.id, "alt_ar", e.target.value)}
                placeholder="نص بديل للصورة (للوصول)…"
                dir="rtl"
                disabled={disabled}
              />
              <div className="flex items-center justify-between">
