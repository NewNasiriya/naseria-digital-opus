...
-- 1) Switch is_media_publicly_visible to SECURITY INVOKER so it no longer
--    runs with elevated privileges when called by anon/authenticated.
--    All referenced tables have appropriate anon/authenticated SELECT policies
--    for published rows, so results are equivalent for legitimate access.
...
-- 2) Replace the broad storage read policy with one that checks the
--    underlying media row is referenced by published content.
DROP POLICY IF EXISTS "storage: public read media" ON storage.objects;

CREATE POLICY "storage: public read referenced media"
ON storage.objects
FOR SELECT
TO public
USING (
  bucket_id = ANY (ARRAY['media'::text, 'documents'::text])
  AND EXISTS (
    SELECT 1
    FROM public.media m
